...
export function registerEVMPrompts(server: McpServer) {
  ...
  server.prompt(
    ...
    "Explore information about a specific block",
    {
      ...
      network: z.string().optional().describe("Network name (e.g., 'base', 'optimism') or chain ID. Supports all EVM networks. Defaults to the configured default network.")
    },
  ...
  server.prompt(
    ...
    {
      address: z.string().describe("EVM address to analyze"),
      network: z.string().optional().describe("Network name (e.g., 'base', 'optimism') or chain ID. Supports all EVM networks. Defaults to the configured default network.")
    },
  ...
  server.prompt(
    ...
    "Get guidance on interacting with a smart contract",
    {
      ...
      network: z.string().optional().describe("Network name or chain ID. Defaults to the configured default network.")
    },
  ...
  server.prompt(
    ...
    {
      networkList: z.string().describe("Comma-separated list of networks to compare (e.g., 'base,optimism')")
    },
    ({ networkList }) => {
      ...
      return {
        messages: [{
          ...
          content: {
            type: "text",
            text: `Please compare the following EVM networks: ${networks.join(', ')}. Include information about their gas fees, transaction speed, security, and any other relevant differences.`
          }
        }]
      };
    }
