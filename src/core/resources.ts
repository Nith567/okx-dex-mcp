...
export function registerEVMResources(server: McpServer) {
  ...
  // OKX DEX API Resources
  
  // Get OKX supported tokens for a specific network
  server.resource(
    "okx_dex_supported_tokens",
    new ResourceTemplate("okx://{network}/dex/tokens", { list: undefined }),
    async (uri, params) => {
      try {
        const network = params.network as string;
        const chainId = await services.getChainId(network);
        ...
        return {
          contents: [{
            uri: uri.href,
            text: `Error fetching OKX supported tokens: ${error instanceof Error ? error.message : String(error)}`
          }]
        };
      }
    }
  ...
  // Default OKX supported tokens (Base mainnet)
  server.resource(
    "default_okx_dex_tokens",
    "okx://dex/tokens",
    async (uri) => {
      try {
  ...
  // Find OKX token by symbol
  server.resource(
    "okx_token_by_symbol",
    new ResourceTemplate("okx://{network}/dex/token/symbol/{symbol}", { list: undefined }),
    async (uri, params) => {
      try {
        const network = params.network as string;
        const symbol = params.symbol as string;
        const chainId = await services.getChainId(network);
        const token = await services.findTokenBySymbol(chainId, symbol);
          ...
          return {
            contents: [{
              ...
              text: JSON.stringify({
                ...
                symbol,
                found: false,
                message: `Token with symbol '${symbol}' not found on ${network}`
              }, null, 2)
            }]
          };
        }
        ...
        return {
          contents: [{
            uri: uri.href,
            text: `Error finding OKX token by symbol: ${error instanceof Error ? error.message : String(error)}`
          }]
        };
      }
    }
  ...
  // Find OKX token by contract address
  server.resource(
    "okx_token_by_address",
    new ResourceTemplate("okx://{network}/dex/token/address/{contractAddress}", { list: undefined }),
    async (uri, params) => {
      try {
        const network = params.network as string;
        const contractAddress = params.contractAddress as string;
        const chainId = await services.getChainId(network);
        const token = await services.findTokenByAddress(chainId, contractAddress);
          ...
          return {
            contents: [{
              ...
              text: JSON.stringify({
                ...
                contractAddress,
                found: false,
                message: `Token with address '${contractAddress}' not found on ${network}`
              }, null, 2)
            }]
          };
        }
        ...
        return {
          contents: [{
            uri: uri.href,
            text: `Error finding OKX token by address: ${error instanceof Error ? error.message : String(error)}`
          }]
        };
      }
    }
